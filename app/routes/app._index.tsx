...
export default function Index() {
  ...
  const normalizeScanResultType = (data: any): ScanResultType => {
    if (data?.resultType === "APPLIED") return "APPLIED";
    if (data?.resultType === "NO_CRITICAL_ISSUE_WITH_REPORT") {
      return "NO_CRITICAL_ISSUE_WITH_REPORT";
    }
    return "SUGGESTION_ONLY";
  };

  const buildFallbackGrowthReport = (
    data: any,
    product: ProductScanResult | null,
    resultType: ScanResultType,
  ): ManualGrowthReport => {
    const issueSignal = getIssueSignal(product);
    const productName = product?.title || "this product";

    if (resultType === "APPLIED") {
      return {
        resultType,
        summary:
          data?.summary ||
          `FeedPilot applied a supported safe fix for ${productName}.`,
        recommendedAction:
          data?.recommendedAction ||
          "Review the refreshed history and continue scanning for product growth gaps.",
    ...
    if (resultType === "NO_CRITICAL_ISSUE_WITH_REPORT") {
      return {
        resultType,
        summary:
          data?.summary || "No critical safe-fix issue found in this scan.",
        recommendedAction:
          data?.recommendedAction ||
    ...
    return {
      ...
      summary:
        data?.summary ||
        `FeedPilot found a product growth gap for ${productName}: ${issueSignal}.`,
      recommendedAction:
        data?.recommendedAction ||
        "Review the signal and use a paid plan when you are ready to apply supported safe fixes.",
      upgradeRequired: Boolean(data?.upgradeRequired),
      usageConsumed: false,
    };
  };
  ...
  const showManualGrowthReport = (
    ...
    const resultType = normalizeScanResultType(data);
    const appliedAndRecorded = isAppliedAndRecordedResponse(data);
    const growthReport =
      getGrowthReport(data) ||
      buildFallbackGrowthReport(data, product, resultType);

    setManualScanResult({
      resultType,
      usageConsumed: appliedAndRecorded,
      safeFixApplied: appliedAndRecorded,
      growthReport,
      product,
    });
    setEmptyRunMessage("");
